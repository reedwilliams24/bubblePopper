...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.prototype.handleKeyEvent = function (key) {
	  if (key === "Left" || key === "Right") {
	    this.rotateAngle(key);
	  } else {
	    this.shootBubble();
	  }
	};
	
	Game.prototype.rotateAngle = function (direction) {
	
	  if (direction === "Left") {
	    this.pointerAngle = Math.max(this.pointerAngle-5, 5);
	  } else {
	    this.pointerAngle = Math.min(this.pointerAngle+5, 175);
	  }
